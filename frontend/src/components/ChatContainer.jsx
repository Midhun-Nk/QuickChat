...
const ChatContainer = () => {
  ...
  const handleSendImage = (e) => {
    ...
    reader.onloadend = async () => {
      ...
      setPreviewImage(null); // remove preview after sending
          ...
          {previewImage && (
            <div className="relative inline-block mr-2">
              <img
                src={previewImage}
                alt="preview"
                className="max-w-[120px] rounded-md border border-gray-600"
              />
              <button
                onClick={() => setPreviewImage(null)}
                className="absolute top-0 right-0 bg-gray-800 text-white w-5 h-5 rounded-full text-xs flex items-center justify-center"
              >
                ×
              </button>
            </div>
